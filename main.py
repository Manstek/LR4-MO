...
def lm(data_train, data_test, formula, y_test, graph_desc):
    ...
    plot_predictions(y_test, predictions, 'Comparison of Actual vs Predicted for ' + graph_desc)
...
glm_h = glm(X_train, X_test, y_ml_train_h, y_ml_test_h, formula_h, 'Y1 - GLM')

formula_c = "Y2 ~ X1+X2+X3+X4+X5+X6+X7+X8"
glm_c = glm(X_train, X_test, y_ml_train_c, y_ml_test_c, formula_c, 'Y2 - GLM')
...
nm_h = nm(X_ml_train_h, X_ml_test_h, y_ml_train_h, y_ml_test_h, 'Y1 - Neural Network')
nm_c = nm(X_ml_train_c, X_ml_test_c, y_ml_train_c, y_ml_test_c, 'Y2 - Neural Network')
...
def mape(train_x, test_x, train_y, test_y, graph_desc, use_mape_weights=False):
    ...
    plot_predictions(test_y, predictions, 'Comparison of Actual vs Predicted for ' + graph_desc)

    mae = mean_absolute_error(test_y, predictions)
    mse = mean_squared_error(test_y, predictions)
    mape_val = np.mean(np.abs((test_y - predictions) / (np.abs(test_y) + eps))) * 100
...
mape_h = mape(X_ml_train_h, X_ml_test_h, y_ml_train_h, y_ml_test_h, 'Y1 - Quantile Regression')
mape_c = mape(X_ml_train_c, X_ml_test_c, y_ml_train_c, y_ml_test_c, 'Y2 - Quantile Regression')
